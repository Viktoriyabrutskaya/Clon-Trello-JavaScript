import { elementBuilder, buttonBuilder } from "./modul.js";

let listTodo = document.querySelector(".todo").querySelector(".task__list"); // Список колонки Todo
let listProgress = document
  .querySelector(".progress")
  .querySelector(".task__list"); // Список колонки Progress
let listDone = document.querySelector(".done").querySelector(".task__list"); // Список коллонки Done
let counterTodo = document
  .querySelector(".todo")
  .querySelector(".topic__count"); // Точка для счётчика тасков в Todo
let counterProgress = document
  .querySelector(".progress")
  .querySelector(".topic__count"); // Точка для счётчика тасков в Progress
let counterDone = document
  .querySelector(".done")
  .querySelector(".topic__count"); // Точка для счётчика тасков в Done
...
overTask.addEventListener("click", (event) => {
  let eventTouch = event.target.className;
  if (eventTouch === "overload__confirm") {
    // По клику на кнопку Confirm:
...
function taskRender(el, i) {
  ...
  let taskDescription = elementBuilder(
    "div",
    "task__description",
    el.description
  );
...
function renderTaskForm() {
  ...
  taskElement.appendChild(
    elementBuilder("label", "inputDescription", "Description")
  );
  ...
  taskElement.appendChild(
    elementBuilder("button", "taskForm__cancel", "Cancel")
  );
  taskElement.appendChild(
    elementBuilder("button", "taskForm__confirm", "Confirm")
  );
  ...
  taskElement.addEventListener("click", (event) => {
    ...
    if (eventTouch === "taskForm__cancel") {
      // по клику на cancel удаляем модальное окно
      taskForm.classList.toggle("visible");
    }
    if (eventTouch === "taskForm__confirm") {
      // по клику на confirm записываем новый таск и удаляем модальное окно
      storeTask();
      taskForm.classList.toggle("visible");
    }
  });
...
function storeTask() {
  ...
  let textDate = `${dateInfo.getDate()} : ${
    dateInfo.getMonth() + 1
  } : ${dateInfo.getFullYear()}`; // дата
...
async function getUsers() {
  userBase = await fetch("https://jsonplaceholder.typicode.com/users/").then(
...
function userInfoRenderList(users) {
  ...
  tempUsers.forEach((el) => {
    ...
    userElement.appendChild(
      elementBuilder("p", "users__tasks", `${el.counter}`)
    );
  ...
  if (selectedUser !== "") {
    ...
    taskBase.forEach((el) => {
      if (el.user === selectedUser) {
        usersTasksDisplay.appendChild(
          elementBuilder("p", "users__taskName", `${el.title}`)
        );
      }
    });
  }
